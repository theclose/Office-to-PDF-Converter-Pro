...
Usage:
    python ai_autofix_direct.py --results=test_results.xml
    python ai_autofix_direct.py --results=test_results.xml --output=fixes.md
"""
...
class DirectModeAutoFix:
    """Generates prompts for AI assistant to fix tests."""
    
    def load_source_code(self, file_path: str, line: int, context: int = 10) -> str:
        """Load source code around the error line."""
        try:
            # Try to find the file
            for base in ['.', 'core', 'utils', 'tests', 'ui', 'scripts']:
    ...
    def generate_prompt(self, failures: List[TestFailure], max_failures: int = 10) -> str:
        ...
        prompt = """# 🔧 AI AutoFix - Test Failures Analysis
...
"""
        ...
        for cat, items in categories.items():
            prompt += f"| {cat} | {len(items)} | {'Test Issue' if 'Test' in cat else 'Potential Bug'} |\n"
...
"""
    ...
    def _categorize_error(self, failure: TestFailure) -> str:
        ...
        if 'nameerror' in msg or 'name' in msg and 'not defined' in msg:
        ...
        elif 'assertionerror' in msg and 'expected' in msg:
            return "Wrong Assertion Type"
    ...
    def _suggest_fix_template(self, failure: TestFailure, category: str) -> str:
        ...
        templates = {
            ...
            "Wrong Assertion Type":
                "Update expected types in parametrize decorator to match actual return type.\n",
            "Missing Attribute":
...
def main():
    parser = argparse.ArgumentParser(description="AI AutoFix - Direct Mode")
    ...
    if not args.results or not Path(args.results).exists():
        ...
        print("Usage: python ai_autofix_direct.py --results=test_results.xml")
        sys.exit(1)
